...
All physical choices are RECOMMENDATION implementing AD-1–AD-11. PostgreSQL is approved. No DDL has been applied.
...
- Logical/API mapping: AcademicIdentityBinding = identity_bindings; SubmissionReceipt projects submission_requests; Submission is only the confirmed submissions row. API maximum maps to assignment_versions.max_score; snapshot capture_state maps to snapshots.state; deletion_state projects the current deletion operation; TestRun.sha maps to head_sha. Immutable API records expose row_version=1 as a projection constant, never a mutable evidence field.
...
| classrooms | T | organization_id FK github_organizations, name text, slug text, timezone text, academic_period text, status text, academic_closed_at? | U institution,slug; draft/active/archived is separate from closure |
...
| accepted_assignments | T | assignment_id FK assignments, roster_entry_id FK roster_entries, binding_id FK identity_bindings, accepted_at, acceptance_version_id FK assignment_versions, next_revision bigint | U assignment,roster; immutable accepting binding; assignment/course matches roster |
...
| submission_requests | T | acceptance_id FK accepted_assignments, actor_id FK users, binding_id FK identity_bindings, sha text, branch text, received_at, effective_deadline?, assignment_version_id FK assignment_versions, extension_id? FK assignment_extensions, policy_version text, validation_state text, idempotency_record_id FK idempotency_records | U idempotency record; immutable receipt columns; separate validation outcomes |
...
| submissions | T | request_id FK submission_requests, acceptance_id FK accepted_assignments, revision bigint, confirmed_at, sha text, assignment_version_id FK assignment_versions | U request; U acceptance,revision; immutable exact request SHA; create only on confirm |
...
| deletion_operations | T | snapshot_id FK snapshots, generation bigint, state text, policy_id FK retention_policies, reason text, claimed_at?, primary_deleted_at?, verified_at?, provider_evidence jsonb | U snapshot,generation; committed fence blocks conflicting new retain/publication action |
| deletion_tombstones | T | snapshot_id FK snapshots, object_identity jsonb, deletion_operation_id FK deletion_operations, recorded_at, export_checkpoint text | Append-only; independently backed up, replay before restore access |
...
Composite FK alone prevents tenant mismatch, not wrong course within the same tenant. Add composite unique parent keys and matching FK where possible: assignment `(tenant,id,classroom_id)`; acceptance `(tenant,id,assignment_id,roster_entry_id)`; request `(tenant,id,acceptance_id,sha,assignment_version_id)`; submission references that tuple. Evaluation/publication use transaction validation and deferred constraint triggers to enforce same submission/evaluation/acceptance and immutable scale. Triggers are migration-owned and tested with direct SQL attempts, not only API tests.

Identity correction locks active profile and current binding; teacher must have access to all affected active course enrollments, otherwise institution admin must grant scoped correction authority to an authorized teacher. A single-course teacher cannot implicitly rewrite an institution-wide identity used elsewhere. No historical author/SHA/grade is reassigned. Binding/account changes enqueue GitHub access reconciliation. Proposed reverse uniqueness prevents one active user/account from claiming two profiles in the same institution; exception requires explicit later policy, not ad hoc override.

Submit intake transaction locks acceptance/policy, samples database `clock_timestamp()` after required locks (not client timestamp or transaction-start time), checks current profile binding and invitation-independent enrollment, snapshots effective deadline/config/extension, persists request/idempotency/outbox, then responds. Deadlines use `received_at <= effective_deadline`; null deadline means not_applicable, not on_time. DB lock/wait time before durable intake is not retrospectively erased; failed persistence has no receipt. Confirmation locks request/acceptance and allocates revision counter, creates submission and snapshot metadata/outbox atomically. Requested receipt fields never mutate.

Publication transaction locks `current_grades` then affected snapshot retention rows in deterministic order. Verify expected current generation and draft version; snapshot scale/feedback; append publication; update pointer; extend retention using maximum previous retain_until and publication+12 calendar months; append audit/outbox. Withdrawal uses same pointer CAS, appends event then clears pointer. It does not shorten retention. Active purge fence causes publication to return `EVIDENCE_PURGE_IN_PROGRESS`; after verified deletion, publication requires explicit acknowledgement of unavailable content and preserves tombstone rather than pretending to restore bytes. This is a proposed recovery rule, surfaced in risks.

## Indexes and query boundaries

Index every referencing composite FK; PostgreSQL does not create FK indexes automatically. Hot indexes: memberships `(tenant,user,status)`; roster `(tenant,classroom,status)`; pending link requests `(tenant,classroom,status,created_at)`; assignments `(tenant,classroom,status)`; requests `(tenant,acceptance,received_at,id)`; submissions `(tenant,acceptance,revision DESC)`; runs `(tenant,repository,head_sha,completed_at)`; publications `(tenant,acceptance,published_at,id)`; snapshots `(tenant,state,retain_until)`; holds `(tenant,snapshot) WHERE released_at IS NULL`; outbox `(state,created_at) WHERE completed_at IS NULL`; inbox `(state,received_at)`; quota reservation `(lease_until) WHERE state='reserved'`. Avoid unbounded whole-course join exports on HTTP threads. Pagination uses stable `(created_at,id)` cursors and fixed allowed sort fields.
