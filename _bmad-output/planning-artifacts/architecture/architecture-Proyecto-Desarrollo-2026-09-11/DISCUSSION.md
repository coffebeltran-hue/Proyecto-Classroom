...
## 2026-09-17 chronological supersession — Juan's repair decisions

Earlier discussion is retained as historical reasoning. This entry supersedes older proposed receipt boundaries, acknowledgement-only post-deletion publication and statements that the first independent review is still pending.

DECISION AD-5 clarification: trusted backend UTC candidate received_at at complete bounded request/server-bound context ingress before internal DB pool/lock/queue waiting; official only on durable intake commit. No surviving intake means no fabricated/backdated receipt from volatile time. Persist receipt, persistence-stage and confirmation times, deadline/policy and provenance separately. Clock/policy ordering uncertainty yields needs_review, not automatic late. Client/commit/TCP/first-byte/arbitrary proxy time is not authoritative. R-01 historical branch observation and TTL remain PROPOSED / RR-04.

DECISION AD-7 clarification: default ordinary publication after verified historical-code deletion is blocked. Only an exact scoped institutional exception permits the teacher to publish, retaining scope, authorizer, publishing teacher, mandatory reason, remaining basis, student-safe explanation and immutable unavailable-code/deletion/publication history. Institutional authority grants but never publishes the grade. No restored snapshot or retroactive twelve-month byte promise. Post-deletion recapture is OUT OF MVP; future capability needs separate approval and new provenance.

Juan authorized ACR-001–011 bounded documentary repairs, including initial/reapproval affected-course authority, uncertain per-account GitHub attempts, cancel-aware independent journal, course/purge lock, intake sequence, request collections, quota sweep, session account, CAS and explicit resolutions, independent alerts. Exact current contracts are in DATA-MODEL, OPERATIONS, SECURITY, API and generated OpenAPI; the original repair proposal remains a historical proposal. AD-1–11 stay adopted with the two express clarifications; AD-12–18 stay PROPOSED despite revision authorization. AD-17 remains conditional on RR-03. OQ-10–13/RR-01–08 stay open. Original Anti-Consensus review is complete and preserved; corrected package awaits targeted independent re-review. No implementation/runtime/provider resources or operational closure authorized.
